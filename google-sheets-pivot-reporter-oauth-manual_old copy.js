...
<body>
  ...
  ${process.env.ENABLE_PRORATION_CALC === 'true' ? `
  <button class="tab-button" onclick="switchTab('proration-calc')">
    <svg width="14" height="14" viewBox="0 0 32 32" style="vertical-align:middle;margin-right:5px;" fill="none">
      <rect width="32" height="32" rx="6" fill="#00AC69"/>
      <text x="50%" y="50%" dominant-baseline="central" text-anchor="middle"
            font-family="Arial Black,sans-serif" font-weight="900"
            font-size="14" fill="#000">NR</text>
    </svg>
  ...
  ` : ''}
  ${process.env.ENABLE_PRORATION_CALC === 'true' ? `   
      <div id="proration-calc" class="tab-content">
        ${this.generateProrationTabHTML()}
      </div>
      ` : ''}                                              
  ...
  async fetchJiraIssues() {
    try {
      ...
      // Debug: show actual priority names from JIRA
      ...
      const vthogaru_tickets = this.jiraBugs.filter(bug => 
        (bug.reporterEmail && bug.reporterEmail.includes('vthogaru@newrelic.com')) ||
        (bug.assigneeEmail && bug.assigneeEmail.includes('vthogaru@newrelic.com')) ||
  ...
  generateBugReportsHTML() {
    ...
    const e2oBugs = this.jiraBugs.filter(bug => (bug.labels || []).includes('e2e_o2c_bugs'));
const p1Count = e2oBugs.filter(b => b.priority === 'P1 - Critical').length;
const p2Count = e2oBugs.filter(b => b.priority === 'P2 - High').length;
const p3Count = e2oBugs.filter(b => b.priority === 'P3 - Medium').length;
const p4Count = e2oBugs.filter(b => b.priority === 'P4 - Low').length;
    ...
    return `
      ...
      <div class="bug-metric-card" style="border-top: 4px solid #dc2626;">
    <h3 style="color: #dc2626; margin: 0 0 5px 0;">🔴 P1 - Critical</h3>
    <div style="font-size: 2.5em; font-weight: 700; color: #dc2626;">${p1Count}</div>
    <div style="font-size: 0.8em; color: #6b7280;">e2e_o2c_bugs</div>
  </div>
  <div class="bug-metric-card" style="border-top: 4px solid #ea580c;">
    <h3 style="color: #ea580c; margin: 0 0 5px 0;">🟠 P2 - High</h3>
    <div style="font-size: 2.5em; font-weight: 700; color: #ea580c;">${p2Count}</div>
    <div style="font-size: 0.8em; color: #6b7280;">e2e_o2c_bugs</div>
  </div>
  <div class="bug-metric-card" style="border-top: 4px solid #ca8a04;">
    <h3 style="color: #ca8a04; margin: 0 0 5px 0;">🟡 P3 - Medium</h3>
    <div style="font-size: 2.5em; font-weight: 700; color: #ca8a04;">${p3Count}</div>
    <div style="font-size: 0.8em; color: #6b7280;">e2e_o2c_bugs</div>
  </div>
  <div class="bug-metric-card" style="border-top: 4px solid #16a34a;">
    <h3 style="color: #16a34a; margin: 0 0 5px 0;">🟢 P4 - Low</h3>
    <div style="font-size: 2.5em; font-weight: 700; color: #16a34a;">${p4Count}</div>
    <div style="font-size: 0.8em; color: #6b7280;">e2e_o2c_bugs</div>
  </div>
</div>
  ...
  generateRegressionReportsHTML() {
    if (!process.env.ENABLE_BUG_REPORTS || process.env.ENABLE_BUG_REPORTS !== 'true') {
     ...
     const regressionBugs = this.jiraBugs.filter(bug => (bug.labels || []).includes('o2c_regression'));
  const rp1Count = regressionBugs.filter(b => b.priority === 'P1 - Critical').length;
  const rp2Count = regressionBugs.filter(b => b.priority === 'P2 - High').length;
  const rp3Count = regressionBugs.filter(b => b.priority === 'P3 - Medium').length;
  const rp4Count = regressionBugs.filter(b => b.priority === 'P4 - Low').length;
    ...
    <div class="bug-metric-card" style="border-top: 4px solid #dc2626;">
      <h3 style="color: #dc2626; margin: 0 0 5px 0;">🔴 P1 - Critical</h3>
      <div style="font-size: 2.5em; font-weight: 700; color: #dc2626;">${rp1Count}</div>
      <div style="font-size: 0.8em; color: #6b7280;">o2c_regression</div>
    </div>
    <div class="bug-metric-card" style="border-top: 4px solid #ea580c;">
      <h3 style="color: #ea580c; margin: 0 0 5px 0;">🟠 P2 - High</h3>
      <div style="font-size: 2.5em; font-weight: 700; color: #ea580c;">${rp2Count}</div>
      <div style="font-size: 0.8em; color: #6b7280;">o2c_regression</div>
    </div>
    <div class="bug-metric-card" style="border-top: 4px solid #ca8a04;">
      <h3 style="color: #ca8a04; margin: 0 0 5px 0;">🟡 P3 - Medium</h3>
      <div style="font-size: 2.5em; font-weight: 700; color: #ca8a04;">${rp3Count}</div>
      <div style="font-size: 0.8em; color: #6b7280;">o2c_regression</div>
    </div>
    <div class="bug-metric-card" style="border-top: 4px solid #16a34a;">
      <h3 style="color: #16a34a; margin: 0 0 5px 0;">🟢 P4 - Low</h3>
      <div style="font-size: 2.5em; font-weight: 700; color: #16a34a;">${rp4Count}</div>
      <div style="font-size: 0.8em; color: #6b7280;">o2c_regression</div>
    </div>
  </div>
    ...
    return `
      <style>
        .proration-tab-wrap {
          ...
          border-radius: 12px;
          overflow: hidden;
          border: 1.5px solid rgba(0, 172, 105, 0.25);
          background: #0c0f18;
        }
        .proration-tab-topbar {
          display: flex;
          align-items: center;
          justify-content: space-between;
          padding: 10px 18px;
          background: linear-gradient(135deg, #071a10, #0c0f18);
          border-bottom: 1px solid rgba(0, 172, 105, 0.2);
          flex-shrink: 0;
        }
        .proration-tab-title {
          display: flex;
        ...
        .proration-tab-title .dot {
          width: 8px; height: 8px;
          background: #1CE783;
          border-radius: 50%;
          box-shadow: 0 0 8px #1CE783;
        ...
        .proration-tab-btn:hover { transform: translateY(-1px); }
        .proration-tab-btn.outline {
          border-color: rgba(0, 172, 105, 0.35);
          color: #1CE783;
          background: transparent;
        }
        .proration-tab-btn.outline:hover {
          background: rgba(0, 172, 105, 0.1);
        }
        .proration-tab-btn.solid {
          border-color: #00AC69;
          background: linear-gradient(135deg, #00AC69, #1CE783);
          color: #000;
        }
        .proration-tab-iframe {
          flex: 1;
          width: 100%;
          border: none;
          background: #0c0f18;
      ...
      <div class="proration-tab-wrap">

        <div class="proration-tab-topbar">
          <div class="proration-tab-title">
            <span class="dot"></span>
            📐 Proration Calculator · proration-v2.html
          </div>
          <div class="proration-tab-actions">
            <button class="proration-tab-btn outline"
              onclick="document.getElementById('proration-iframe').src = document.getElementById('proration-iframe').src">
              🔄 Reload
            </button>
            <a class="proration-tab-btn solid"
              href="./proration-v2.html"
              target="_blank"
              rel="noopener">
              ↗ Open Full Screen
            </a>
        ...
        <iframe
          id="proration-iframe"
          class="proration-tab-iframe"
          src="./proration-v2.html"
          title="NR Proration Calculator"
          loading="lazy">
        </iframe>

        <div class="proration-tab-note">
          ⓘ This tab loads <strong style="color:#6b7a99">proration-v2.html</strong> independently.
          Edit that file freely — changes apply here automatically on reload.
          No impact to test results, pivot table, or any other report section.
        </div>
